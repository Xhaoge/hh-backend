...
class User(db.Model):
    ...
    username = db.Column(db.String(128), doc='账号', nullable=False)
    password = db.Column(db.String(128), doc='账号密码', nullable=False)
    phone = db.Column(db.String(20), doc='手机号码', nullable=False, unique=True)
    isAdmin = db.Column(db.Boolean, doc='是否管理员', default=False)
    userRestrict = db.Column(db.String(32), doc='用户权限', nullable=False)
